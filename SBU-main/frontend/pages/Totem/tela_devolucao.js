document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        try {
            const resposta = await fetch("http://localhost:3000/api/devolucao/devolucaoLivro", {
                method: "POST",
            ...
            if (!resposta.ok) {
                mostrarToast("toast-erro", data.erro || "Erro ao registrar devolução!");
                return;
            }

            mostrarToast("toast-sucesso", "Devolução realizada com sucesso!");
